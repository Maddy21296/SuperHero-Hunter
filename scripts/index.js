...
function getCharacterList(e){
    ...
    fetch(`https://gateway.marvel.com/v1/public/characters?nameStartsWith=${searchInputTxt}&ts=1694943615713&apikey=1da26789400d41d2a05e193571341a2e&hash=ed0bfd721d70b3f389b88f04c42dc5b4`)
...
function getCharacterInfo(e){
    ...
    if(e.target.classList.contains('info-btn')){
        ...
        fetch(`https://gateway.marvel.com/v1/public/characters/${hero.dataset.id}?&ts=1694943615713&apikey=1da26789400d41d2a05e193571341a2e&hash=ed0bfd721d70b3f389b88f04c42dc5b4`)
        .then(response => response.json())
...
function characterModal(heroData){
    ...
    // get character stories information
    var storiesName = hero.stories.items;
    ...
    // iterate through stories items and add them to span element
    for (const key in storiesName) {

        const span = document.createElement('span');
        span.textContent = storiesName[key].name;
        storiesDiv.appendChild(span);
        if (key != (storiesName.length - 1)) {
            storiesDiv.appendChild(document.createTextNode(', '));
        }
